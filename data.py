...
class MethylationDataset:
    """
    Simple class for DNA methylation data.
    ...
    def __init__(self, methylation, phenotype, _meth_path=None, _pheno_path=None):
        ...
        self.meth_matrix = np.asfortranarray(self.meth_table.values.T)
        self.age = np.asfortranarray(self.pheno_table.loc[:, 'age'].values)
...
class TissueSimilarity:
    ...
    def __init__(self, _base_path):
        self._path_similarity = os.path.join(_base_path, "tissueSimilarityFromNaturePaper/cis_translated.csv")
